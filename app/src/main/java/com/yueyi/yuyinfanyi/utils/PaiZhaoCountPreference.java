...
public class PaiZhaoCountPreference {
    public final static String TAG = PaiZhaoCountPreference.class.getSimpleName();

    private static PaiZhaoCountPreference mInstance;

    private SharedPreferences mPreferences;

    public static final String USER_LOGIN = "paizhao_count";
    private PaiZhaoCountPreference(Context context) {
        mPreferences = context.getSharedPreferences(USER_LOGIN, 0);
    }

    public synchronized static PaiZhaoCountPreference getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new PaiZhaoCountPreference(context);
        }
        return mInstance;
    }
    //检测是否能够使用语言翻译
